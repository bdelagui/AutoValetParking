# Class to define the test configuration at the start of each test run in which system and environment transitions are given as input
# Apurva Badithela
# 8/3/20
...
class test_run_configuration:
    def __init__(self, Tsys):
        self.nodes = Tsys.keys()
        self.transitions = Tsys.copy()
        self.graph = []
        self.final_goal= None
        self.propositions = None
        self.create_graph()
        self.adjacency = nx.adjacency_matrix(self.graph[0]) # Adjacency matrix describing where the static obstacles are
...
# % ======================= Updating the matrix representing the static obstacle grid configuration ================= % #
    ...
    def update_matrix(self, cut_transitions):
        M = [self.adjacency]
        for ii in range(len(cut_transitions)):
            M_ii = M[ii].copy()
            cut_trans = cut_transitions[ii]
            for c in cut_trans:
                M_ii[c[0], c[1]] = -1
            M.append(M_ii)
        self.adjacency = M[-1]
    ...
    # ToDO: animate_transition dynamics: if time permits
    ...
    # # Base plot is on the adjacency matrix in the absence of a gridworld layout
    # # ToDo: Think about base plot here
